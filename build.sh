...
git config --global user.email "846800668@qq.com" #填入GitHub的邮箱地址
git config --global user.name "labnize" #填入GitHub的用户名
git add --all .
git commit -m "Travis CI Auto Builder" #自动构建后的内容将全部以此信息提交
git push --force https://${GitHubKEY}@github.com/labnize/labnize.github.io master  #自动构建后的内容将全部以此信息提交
